...
def load_and_inspect(directory_path, sample_fraction=0.1):
    ...
    try:
        ...
        print(
            f"Found {len(csv_files)} files: {[os.path.basename(f) for f in csv_files]}"
        )
        ...
        for file in csv_files:
            ...
            try:
                ...
                df = pd.read_csv(
                    file, usecols=lambda c: c in usecols, dtype=dtypes, low_memory=False
                )
                ...
                if sample_fraction < 1.0:
                    print(
                        f"  Sampling {sample_fraction * 100}% of {os.path.basename(file)}..."
                    )
                    df = stratified_sample_with_min(
                        df, frac=sample_fraction, min_samples=100
                    )
